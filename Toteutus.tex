...
\verb|RORegEx| on rajapintaluokka, joka sisältää kirjastokäyttöliittymän säännöllisen lausekkeen matchausta varten. Luokka konstruoi halutusta regexistä ja halutusta stringistä \verb|NSTextCheckingResult|-olion, joka sisältää kaikki stringin sisältämät regex-matchit eli aloitusindeksit ja pituudet järjestyksessä.

\subsection{RONFA-luokka}

\verb|RONFA| on luokka, joka sisältää itse regex-tulkin konstruointilogiikan sekä myös matchauslogiikan. Luokka initialisoidaan halutulla regexillä, jonka jälkeen \verb|findMatch:|-kutsulla voidaan saada halutusta stringistä ensimmäinen regexiä vastaava match. 

\verb|RONFA|-luokan toteutus jakautuu vastaavasti initialisointi- ja matchausmetodeihin. Koska \verb|RONFA| on toteutus epädeterministiselle tilakoneelle, joka vastaa haluttua regexiä, itse olion initialisointi eli tilakoneen konstruointi on eniten koodirivejä sisältävä metodi. \verb|initWithRegEx:| käy läpi regexin merkki kerrallaan ja lisää tilakoneen alkutilaan uusia seuraavia tiloja sen mukaan, mikä merkki tai operaattori lausekkeessa tulee vastaan. Näin ollen koko lausekkeen tunnistamislogiikka synnytetään heti olion initialisointivaiheessa. Metodi on sitä monimutkaisempi, mitä monimutkaisempia regex-operaattoreita halutaan toteuttaa.

\verb|RONFA|-luokan \verb|findMatch:|-metodi ajaa tilakonetta alkutilasta lähtien annetulla stringillä. Koska luokka käyttää matchien tallentamiseen itse tilaolioita, tämän metodin alussa tilakone joudutaan konstruoimaan uudestaan edellisten tulosten poistamiseksi.

\subsection{ROState-luokka}

\verb|RONFA| käyttää \verb|ROState|-luokkaa esittämään yhtä mahdollista tilaa. Näin ollen tilan olennaiset ominaisuudet ovat matchaava kirjain (\verb|matchingCharacter|), seuraavat mahdolliset tilat (\verb|nextState| sekä \verb|alternateState|) ja lopputila (\verb|finality|). Näistä \verb|matchingCharacter| voi olla \verb|nil|, jos halutaan matchata mikä tahansa kirjain; \verb|alternateState| taas on \verb|nil|, mikäli tila ylipäänsä on matchaava tila, ja toinen tila, mikäli tila haarautuu. Tällöin \verb|matchingCharacter|-arvoa ei lueta. Lisäksi luokka sisältää joka kierroksella päivitettävät \verb|startIndex| ja \verb|nextStartIndex|-kentät, joihin tilakone tallentaa kulloisenkin matchin aloitusindeksit.

\verb|ROState|-luokkaan ei ollut järkevää lisätä tilakoneen toteutuslogiikkaa, sillä mitkä tahansa operaattorit on toteutettavissa kolmea tilatyyppiä yhdistelemällä; näin ollen yksittäinen \verb|ROState| on lähinnä olio, joka sisältää muutaman kentän sekä viittauksen muihin \verb|ROState|-olioihin. Näistä konstruoituu \verb|RONFA|:n initialisaatiossa puu, joka sisältää viittaukset kaikkiin mahdollisiin tilakoneen tiloihin. Olemassaoleviin tiloihin ei ole muita viittauksia kuin ensimmäisestä tilasta lähtevä, haarautuva linkitetty lista. Operaattoreista riippuen listaan syntyy myös silmukoita ja haarojen yhdistymisiä. Se, mitkä tilat päätyvät kulloinkin tilakoneen \verb|currentStates|-joukkoon, riippuu siitä, mihin tilakoneen haaroihin stringin matchaus etenee.
...
Näiden tilojen tallennus on toteutettu \verb|NSMutableSet|-joukkoon \verb|currentStates|, joka on siis järjestämätön joukko olioita. Mikäli sama tila tulee vastaan saman merkin kohdalla tilakoneen eri haaroissa, tilaa \emph{ei} lisätä useampaan kertaan seuraavaksi käsiteltäviin tiloihin, vaan haarat sulautuvat takaisin yhteen; tämä on olennaista eksponentiaalisen skaalautumisen estämiseksi. Samalla kuitenkin unohtuu se, mitä haaroja pitkin kuhunkin tilaan on päästy. Matchin alkukirjaimen muistamiseksi tilakone tallentaa matchin alkuindeksin matchattuun \verb|ROState|-tilaan ja kopioi alkuindeksiä eteenpäin siirryttäessä. Ns. ahneen matchauksen toteuttamiseksi pienin alkuindekseistä on aina se, joka jää voimaan seuraavassa tilassa.
...
Koska matchaavien tilojen lisäksi kone saattaa sisältää myös haarautuvia tiloja, joista edetään ilman etenemistä stringissä, nämä tilat on otettava huomioon ennen kuin stringissä edetään seuraavaan kirjaimeen. Tämä tapahtuu jokaisen kirjaimen kohdalla \verb|pruneForks|-metodilla. Metodi käy kaikki \verb|currentStates|in sisältämät tilat läpi ja haaran löydettyään lisää tämän haarat \verb|currentStates|iin sekä poistaa alkuperäisen tilan. Näin kirjaimen matchaukseen päädytään vasta siinä vaiheessa, kun kaikki haarautuvat tilat sekä niiden alla olevat haarautuvat tilat on poistettu ja \verb|currentStates| sisältää ainoastaan sellaisia tiloja, joita matchataan stringin nykyiseen kirjaimeen. Itse kirjaimen matchaus on silmukassa eroteltu \verb|matchCharacter|-metodiin, joka huolehtii matchauksen aloitusindeksin tallentamisesta.
...
\caption{\label{fig:skaalautuminen} $n$:n kirjaimen pituisen testistringin matchauksen nopeus $n$:n funktiona toteutetulla algoritmilla (RONFA) sekä Applen NSRegularExpressionin käyttämällä algoritmilla.}
\end{figure}

Lausekkeen $(a?)^n(a)^n$ matchauksen skaalautuvuus on osoitettu testeillä \verb|testPerformancePathologicaln|, missä stringin pituus on $n=5,10,12,15,18,20,25,30,40,60,80.$ Tällöin itse regexin pituudeksi tulee $3n$. Koko \verb|findMatch|-metodin suoritus tulisi kestää ajan $O(3n(3n+n))$ eli $O(n^2)$. Tulosten perusteella skaalautuminen on aavistuksen verran huonompaa kuin neliöllinen; kolmannen asteen polynomi sopii dataan merkittävästi paremmin (korrelaatiokerroin $R^2>0.999$).

Kuutiollisen skaalautumisen syitä pitää etsiä \verb|findMatch|-kutsun tarkemmasta toteutuksesta, joka vaatii matchin aloitusindeksien tallennuksen ja päivityksen jokaisella askeleella. Koska jokaisen käsitellyn ja tulevan tilan aloitusindeksit on päivitettävä tasan kerran riippumatta siitä, kuuluuko tila jompaankumpaan vai kumpaankin joukoista \verb|currentStates| ja \verb|nextStates|, jokainen askel edellyttää \verb|currentStates|- ja \verb|nextStates|-joukkojen yhdistämistä, eli käytännössä n:n kokoisen joukon läpikäymistä kertaalleen duplikaattien eliminoimiseksi.
